...
border
  ...
  >
    ...
    <el-table-column prop="book.title" label="文章" width="230"></el-table-column>
    <el-table-column prop="totalCopies" label="书籍总量" width="100"></el-table-column>
    <el-table-column prop="availableCopies" label="剩余数量" width="100"></el-table-column>
  ...
  <el-dialog v-model="dialogFormVisible" :title="dialogTitle">
    <el-form :model="tableform">
      ...
      <el-form-item label="ISBN" :label-width="100" v-if="is_edit">
        <el-input v-model="tableform.bookIsbn" autocomplete="off"/>
      </el-form-item>
      <el-form-item label="书籍总量" :label-width="100">
        <el-input v-model="tableform.totalCopies" autocomplete="off"/>
      </el-form-item>
      <el-form-item label="剩余数量" :label-width="100">
        <el-input v-model="tableform.availableCopies" autocomplete="off"/>
      </el-form-item>
    </el-form>
    <template #footer>
...
/*
// 发送请求获取数据
axios.get(`http://localhost:8080/bookInventory/findAll/${id}`).then((response) => {
...
//获取当前页数据
const getData = () => {
  axios.get(`http://localhost:8080/bookInventory/findByPage`, {
    params: {
...
const handleSearchName = (val) => {
  if (val.length > 0) {
    axios.get(`http://localhost:8080/bookInventory/search`, {params: {keyword: val}}).then(response => {
...
const dialogOk = () => {
  ...
  const publisherBook = { ...tableform.value, publisherId: id};

  if (dialogType.value === 'add') {
      axios.post("http://localhost:8080/bookInventory/create", publisherBook)
          .then((response) => {
      ...
      axios.put(`http://localhost:8080/bookInventory/uid/${tableform.value.id}`, tableform.value)
          .then((response) => {
...
const delrow = (row) => { // 定义一个函数，用于删除某行数据
  axios.delete(`http://localhost:8080/bookInventory/id/${row.id}`)
      .then(() => {
        getData();
      })
      .catch((error) => {
